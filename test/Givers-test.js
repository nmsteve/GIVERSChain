...
describe("Givers Chain Test", function () {
  ...
  async function deploy() {
    ...
    factorysigner = factory.connect(owner)

    //set Pair
    //const pairAddress = await factorysigner.callStatic.createPair(process.env.giversEdited, process.env.WETH)
    pairAddress = hardhatToken.uniswapV2Pair()
  ...
  describe('Transfers:After swapAndLiquify is enabled', function () {
    ...
    it('Should send fee to Charity and marketing wallet', async function () {
      ...
      const charityBalanceBefore = await provider.getBalance(addr1.address)
      const marketingBalanceBefore = await provider.getBalance(addr2.address)
      const tokenContractBalanceBefore = await provider.getBalance(hardhatToken.address)
     // expect(true).to.be.equal(charityBalanceBefore < marketingBalanceBefore && tokenContractBalanceBefore < marketingBalanceBefore)
      ...
      const charityBalanceAfter = await provider.getBalance(addr1.address)
      const marketingBalanceAfter = await provider.getBalance(addr2.address)
      const tokenContractBalanceAfter = await provider.getBalance(hardhatToken.address)
  ...
  describe("Liquidity", function () {
    ...
    it("Should remove half Liquidity", async function () {
      ...
      await pairsigner.approve(process.env.ROUTER02, BigInt(half));
      await routersigner.removeLiquidityETHSupportingFeeOnTransferTokens(
        hardhatToken.address,
        BigInt(half),
        0,
        0,
      ...
      var ownerBal = await pairsigner.balanceOf(owner.address);
